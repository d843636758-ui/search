...
MOBILE = os.getenv("BROWSER_MOBILE", "true").lower() not in {"0", "false", "no"}
MOBILE_DEVICE = os.getenv("BROWSER_MOBILE_DEVICE", "Pixel 7").strip() or "Pixel 7"

ALLOWED_HOSTS = [
    x.strip().lower()
    for x in os.getenv("BROWSER_ALLOWED_HOSTS", "*").split(",")
    if x.strip()
]

BLOCK_CLICK = re.compile(
    r"(提交订单|确认下单|确认订单|立即下单|去支付|立即支付|确认支付|付款|确认付款|"
    r"转账|充值|提现|确认购买|立即购买|购买|订阅|删除账号|注销账号|"
    r"place\s*order|confirm\s*purchase|pay\s*now|transfer|withdraw|delete\s*account)",
    re.I,
)

BLOCK_INPUT = re.compile(
    r"(验证码|密码|口令|支付|银行卡|身份证|手机号|手机号码|"
    r"card|password|otp|sms|cvv|security\s*code|2fa|two[- ]factor)",
    re.I,
)
...
class Browser:
    ...
    async def start(self):
        async with self.lock:
            ...
            # 关键修复：
            # 原来只是把桌面浏览器 viewport 缩窄，
            # 很多网站仍然识别成桌面 Chromium，
            # 所以会输出固定宽度桌面网页，导致手机前端只能看见半边。
            #
            # 现在直接使用 Playwright 手机设备描述，
            # 同时设置：
            # - mobile User-Agent
            # - is_mobile
            # - touch
            ...
            if MOBILE:
                device = (
                    self.pw.devices.get(MOBILE_DEVICE)
                    or self.pw.devices.get("Pixel 7")
                )
            ...
            lat = os.getenv("BROWSER_LAT", "").strip()
            lng = os.getenv("BROWSER_LNG", "").strip()
            ...
            if (
                START_URL not in {"about:blank", ""}
                and self.page.url in {"about:blank", ""}
            ):
    ...
    async def goto(
        ...
        if mcp_safe:
            ...
            try:
                ...
                # 普通域名不是 IP，所以 ip_address 会抛 ValueError。
                # 我们自己用于禁止私网的异常则继续抛出。
                if "默认禁止" in str(e):
                    raise
    ...
    <div class="grid">
        ...
        >
            ...
            <div class="bar">
                ...
                <button
                    class="primary"
                    onclick="quickGoto('https://h5.waimai.meituan.com/waimai/mindex/home')"
                >
                    美团外卖
                </button>

                <button
                    onclick="quickGoto('https://www.taobao.com/')"
                >
                    淘宝
                </button>

                <button
                    onclick="quickGoto('https://www.jd.com/')"
                >
                    京东
                </button>

                <button
                    onclick="quickGoto('https://www.xiaohongshu.com/')"
                >
                    小红书
                </button>

                <button
                    onclick="post('/api/browser/back')"
                >
                    ← 后退
                </button>

                <button
                    onclick="post('/api/browser/reload')"
                >
                    刷新
                </button>

                <button
                    onclick="scrollByRemote(-650)"
                >
                    ↑
                </button>
        ...
        >
            ...
            <div class="group">
                ...
                <div class="row">
                    ...
                    <button
                        onclick="quickGoto('https://www.baidu.com/')"
                    >
                        百度
                    </button>
            ...
            <div class="group">
                ...
                <div class="row">
                    ...
                    <button
                        onclick="key('Control+A')"
                    >
                        全选
                    </button>

                    <button
                        onclick="key('Backspace')"
                    >
                        删除
                    </button>
